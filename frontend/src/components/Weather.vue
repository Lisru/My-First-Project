...
<template>
<div style="height: 160px" v-loading="!data.success" element-loading-text="正在加载">
  <div style="display: flex;justify-content: space-between;margin: 10px 20px"
    v-if="data.success">
    <div style="font-size: 45px;color: darkorange">
    ...
    <div style="margin-top: 13px">
      ...
      <div style="font-size: 13px;color: grey">{{`${data.location.adm2} ${data.location.name}区`}}</div>
    </div>
  </div>
  ...
  <div style="display: grid;grid-template-columns: repeat(5,1fr);text-align: center">
    <div v-for="item in data.hourly">
      ...
      <div style="font-size: 23px;color: darkorange"><i :class="`qi-${item.icon}-fill`"></i></div>
